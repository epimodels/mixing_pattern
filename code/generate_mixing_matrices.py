...
# Updating the layout by adding a barchart of the antibiotic exposure ratio
...
def process_data(contact_table_filename, antibiotic_profile_filename):
    ...
    data_groupby_hospital = data_contact.groupby('hid_proxy')
...
def prepare_mixing_matrices(data_contact, unit_list_per_hospital, data_antibiotic):
    ...
    df_ab_vs_no_ab_proportion = prepare_ab_vs_no_ab_proportion(data_contact, unit_list_per_hospital, data_antibiotic)
...
def plot_and_manage_javascript_calling(data_age_mixing, data_elix_score_mixing, data_antibiotic_rank_mixing, data_no_antibiotic_ratio, hospitals_list, unit_list_per_hospital):
    ...
    barchart, source_barchart, source_all_barchart = plot_antibiotic_exposure_proportion_barchart(data_no_antibiotic_ratio)

    # dropdown menu for selecting hospitalid
    dropdown_hospitalid = Select(options=list(hospitals_list), value='Hospital-1', title='Select Hospital')
    # dropdown menu for selecting unit within the selected hospital
    updated_hospital_dict = {str(key): value for key, value in unit_list_per_hospital.items()}
    dropdown_unit_names = Select(options=list(updated_hospital_dict['Hospital-1']),value='', title = 'Select Unit')
    ...
    console.log("from dd_hospital: "+unit)
    var data_age = sc_all.data
    //console.log(typeof data_age)
    ...
    var data_antibiotic_exposure_proportion = sc_all_barchart.data
    var colname_percentage = hid+'_'+unit+'_percentage'
    console.log("from dd_hospital: "+colname_percentage)
    sc_barchart.data['no_antibiotic'] = [data_antibiotic_exposure_proportion[colname_percentage][0]]
    console.log("from dd_hospital: "+sc_barchart.data['no_antibiotic'])
    sc_barchart.data['one_antibiotic'] = [data_antibiotic_exposure_proportion[colname_percentage][1]]
    sc_barchart.data['both_antibiotic'] = [data_antibiotic_exposure_proportion[colname_percentage][2]]
    sc_barchart.change.emit()
    ...
    barchart.title.text = "Antibiotic Exposed and Not Exposed Contacts"
    //"Bar chart of antibiotic exposure"
    ...
    console.log("from dd_unit: "+hid)
    console.log("from dd_unit: "+unit)
    var data_age = sc_all.data
    var colname_contact_count = hid+ '_' + unit + '_contact_count'
    var colname_normalized_contact_count = hid+ '_' + unit + '_normalized_contact_count'
    var colname_age_x = hid+ '_' + unit + '_age_x'
    ...
    var data_antibiotic_exposure_proportion = sc_all_barchart.data
    var colname_percentage = hid+'_'+unit+'_percentage'
    console.log("from dd_unit: "+colname_percentage)
    sc_barchart.data['no_antibiotic'] = [data_antibiotic_exposure_proportion[colname_percentage][0]]
    sc_barchart.data['one_antibiotic'] = [data_antibiotic_exposure_proportion[colname_percentage][1]]
    sc_barchart.data['both_antibiotic'] = [data_antibiotic_exposure_proportion[colname_percentage][2]]
    sc_barchart.change.emit()
    ...
    layout = column(row(dropdown_hospitalid, dropdown_unit_names),
        ...
        row(hm_antibiotic_rank, barchart))
